...
foreach my $log (@files) {	#Recorremos log a log
	...
	$my_file=$log;
	open(FICH,">>$log") || die("Can't access to $log.\n");
...
foreach $na (@nacceso){
	...
	@horaguay = split(/\./, $hora);
	print ESCRIBIR "$fecha $horaguay[0] Creation/Modification of $na";
	$i++;
}
...
# Eliminamos por si existe de antes 
system("touch historicofinal.log"); 
system("rm historicofinal.log 2>/dev/null");

my $year = `date +%Y`; #obtenemos el año actual
chop $year;

# AQUI INSERTAMOS EL CAMBIO DE WTMP A FICHERO NORMAL
...
$ant = 0;
$antant =0;
print "The complete log file will contain $cuenta lines.\n";
while ($cuenta > 0) { # Implementar masLogs # Hace la suma de líneas por cada una.
	...
	if ($salto==1){
		$salto=0;
		print ESCRIBIR "JUMP $files[$indice]\n";
	}
	if ($files[$indice] ne "")	{
	...
	if ($cuenta % 1000 == 0) { print "Lines left = $cuenta\n"; }
}
...
my $text = '
\section{Logs overlapping}
The logs and their signatures are:
\begin{itemize}
';
...
sub actualizarRecientes{
	for $i ( 0 .. $#logs){ #i = linea última de los ficheros recientes
		...
		while ($fecha[$i] !~ /((\d{4})-(\d{1,2})-(\d{1,2}))/ ){ #Por si hay alguna línea jodida
			print "date = $fecha[$i], it isn't correct, we take another one (file $i).\n";
			$reciente[$i] = pop(@{$logs[$i]}); 
			($fecha[$i],$horario[$i],@resto) = split(' ',$reciente[$i]);	
		}
