...
my $label1 = Gtk2::Label->new("Select step:");
...
$radiobutton1 = Gtk2::RadioButton->new(undef,"Create image");
#$radiobutton1->set_active(TRUE);
...
$radiobutton4 = Gtk2::RadioButton->new(@group,"Event Viewer");
...
sub aceptar{
	...
	if($radiobutton1 -> get_active()){
		...
		exec("perl principal_en.pl");

 	} elsif ($radiobutton2 -> get_active()){
		#print "Se ha elegido accion 2.\n";
		if ($radiobutton01 -> get_active()){
			#print "Se ha elegido firma md5.\n";
			exec("perl verificar_en.pl $direccionsalida md5");
		} elsif ($radiobutton02 -> get_active()) {
			#print "Se ha elegido firma sha1.\n";
			exec("perl verificar_en.pl $direccionsalida sha1");
		}

	} elsif ($radiobutton3 -> get_active()){
		#print "Se ha elegido accion 3.\n";	
		if ($numimgs =~ "2x2"){
			exec("perl visorimagenes2x2_en.pl $direccionsalida");
		} elsif ($numimgs =~ "3x3"){
			exec("perl visorimagenes3x3_en.pl $direccionsalida");
		} elsif ($numimgs =~ "4x4") {
			exec("perl visorimagenes4x4_en.pl $direccionsalida");
		}
	} elsif ($radiobutton4 -> get_active()){
		#print "Se ha elegido accion 4.\n";
		exec("perl visoreventos_en.pl $direccionsalida");
	}
