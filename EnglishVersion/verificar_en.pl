...
if ( $#ARGV == 1 ) { # El ultimo indice es 1, por lo tanto hay dos elementos
	...
	die "Error, two arguments are needed.\n";
}
...
$window->set_title('Dissection :: Verification');
$window->set_default_size(400, 200);

#Preparación de widgets
my $vbox = Gtk2::VBox->new(FALSE,5);
my $label1 = Gtk2::Label->new("Choose the analysis to do:");
...
$statusbar->push($statusbar->get_context_id('statusbar'),'Checking if it\'s correct the image...');

$op1 = 0;
$op2 = 0;
my $check1 = Gtk2::CheckButton->new("Data carving");
my $check2 = Gtk2::CheckButton->new("Logs overlapping");
...
if (-e $destinodd){
	print "Checking the signature $firma\n";
 
} else {
	# No existe la imagen descomprimida:
	print "Doesn't exist uncompressed image, uncompressing before verifying the signature $firma.\n";
...
if ($firma eq "md5"){
	...
	die "Signature doesn\'t fit: $firma\n";
}

if ($firmacorrecta == 1){
	$statusbar->push($statusbar->get_context_id('statusbar'),'Correct');
} else {
	$statusbar->push($statusbar->get_context_id('statusbar'),'Error, signature doesn\'t fit');
}
...
sub aceptar{
	...
	if ($firmacorrecta == 0){
		print "Signature doesn't fit. You should start a new analysis.\n";
		# Fail, no es correcta
		...
		if (($op1 == 1)and($op2 == 1)){
			...
			## 1.1- Carving
			chdir($dactual) or die "Can't access to $dactual.\n"; #donde estan los .pl
			system("perl carving_en.pl $direccionsalida $firma");

			# 1.2.- Llamar al Visor de Imagenes
			chdir($dactual) or die "Can't access to $dactual.\n"; #donde estan los .pl
			
			if ($aux =~ "3x3"){
				system("perl visorimagenes3x3_en.pl $direccionsalida");
			} elsif ($aux =~ "4x4") {
				system("perl visorimagenes4x4_en.pl $direccionsalida");
			} else {
				system("perl visorimagenes2x2_en.pl $direccionsalida");
			}

			# Realizar Logs + Visor de Eventos
			# 2.1- Recolectar logs
			chdir($dactual) or die "Can't access to $dactual.\n"; #donde estan los .pl
			system("perl logs_en.pl $direccionsalida $firma");
			
			if (-e $auxpdf) { 
				print "Analysis made previously.\n.\n"; 
				goto noEscribe1; 
...
';
			...
			chdir($dactual) or die "Can't access to $dactual.\n"; #donde estan los .pl
			system("perl visoreventos_en.pl $direccionsalida");
